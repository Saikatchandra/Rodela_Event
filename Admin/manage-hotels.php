...
?>
            ...
            <div class="content">
                ...
                <div class="content-header">
                    ...
                    <div class="leftside-content-header">
                        <ul class="breadcrumbs">
                            ...
                            <li><i class="fa fa-home" aria-hidden="true"></i><a href="manage-hotels.php">Manage Hotls</a></li>
                        </ul>
                    </div>
                </div>
                <!-- =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= -->
                ...
                <div class="col-sm-12">
                    <h4 class="section-subtitle text-center"><b>All Hotel's View:</b></h4>
                    <div class="panel">
                     ...
                     // hotel display
                          ...
                          $result = mysqli_query($con, "SELECT * FROM `hotel` LIMIT ". $limit . ','.$parpage);
                      
                          while ($hotel_row = mysqli_fetch_array($result)){
                                        
                    ?>
                                         <tr>
                                         <td><?= $hotel_row['h_id'] ?></td>
                                            <td><?= ucwords($hotel_row['h_name']) ?> </td>
                                            <td><?= ucwords($hotel_row['address']) ?> </td>
                                            <td> <img src="images/hotel-images/<?= $hotel_row['image'] ?>" alt="" width="100px" height="100px" class="fluid"> </td>
                                            <td><?= $hotel_row['description'] ?></td>
                                            <td><?= $hotel_row['price'] ?></td>
                                            
                                            <td><?= date('d-M-y', strtotime($hotel_row['h_date'] ))?> </td>
                                            <!--Button for model-->
                                            
                                            <td>
                                                <a href="delete.php?hoteldelete=<?= base64_encode($hotel_row['h_id']) ?>" class="btn btn-danger" onclick="return confirm('Are you sure delete this hotel?')"><i class="fa fa-trash-o"></i></a>
                                            </td>
                                            <td>
...
<?php
    ...
    if($page > 1){?>
        ...
        <li><a href="manage-hotels.php?page=<?= $page-1 ?>" data-dt-idx="0" tabindex="0">Previous</a></li><li class="paginate_button">
    ...
    for($i = 1; $i <= $total_page; $i++){?>
        <a href="manage-hotels.php?page=<?= $i ?>" aria-controls="basic-table" data-dt-idx="0" tabindex="0" class="btn btn-info <?= $page == $i ? 'active':' '?>"><?= $i ?></a></li><li class="paginate_button">
        <?php
    }
    if( ($i > $page) && ($page < $total_page)){?>
        <a href="manage-hotels.php?page=<?= $page+1 ?>"  aria-controls="basic-table" data-dt-idx="0" tabindex="0" class="btn btn-secondary">Next</a></li><li class="paginate_button next" id="basic-table_next">
